...
App::uses('Aro', 'Model');
class MenuTesterComponent extends Component
{
	...
	public function _carregaActions($menugroup_id)
	{
		...
		if(!empty($consulta)){
			...
			for($i=0; $i<sizeof($consulta); $i++){
				...
				/*$retorno .= '<li style="position: static;" class="">'.
	...
	public function _returnFormatAco($aco)
	{
		...
		/*if($ControllerPlugin['controller'] <> 'bookPoints' and $ControllerPlugin['controller'] <> 'bookPointPrints'){
			pr($retorno);
			exit(0);
		}*/
