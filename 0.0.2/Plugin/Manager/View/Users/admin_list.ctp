...
<div class="main">
	<h2><?php echo __('Usuários Ativos');?></h2>
		...
		<table cellpadding="0" cellspacing="0" id="dataTable1" width="800" class="scroll">
			...
			?>
				<tr>
					...
					&nbsp;</span></td>
					<td class="actions">
					<?php echo $this->Form->postLink(__('Desativar'), array('action' => 'disableuser', 
					$user['User']['id']), null, __('Você tem certeza que deseja desativar o usuário %s?', $user['User']['username'])); ?>
	
					<?php echo $this->Html->link('Visualizar perfil', array('plugin'=>'manager', 'controller'=>'persons', 'admin'=>true, 'action'
					=>'view', $user['User']['person_id'])); ?>
					
					<?php echo $this->Html->link('Editar usuário', array('plugin'=>'manager', 'controller'=>'users', 'admin'=>true, 'action'
					=>'edit', $user['User']['id'])); ?>
					</td>
				</tr>
			<?php 
