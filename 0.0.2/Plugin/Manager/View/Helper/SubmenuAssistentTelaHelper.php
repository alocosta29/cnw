...
class SubmenuAssistentTelaHelper extends AppHelper 
{
	
	public $helpers = array('Manager.MenuAssistentTela');
	...
	public function _showSubmenu($menugroup_id, $role_id)
	{
		...
		if($submenus <> false)
		{
			...
			{
				$retorno .= '<ul class="borda_ger" style="position: static; list-style:none">'.'<li class="title_sub">'.$submenus[$i]['grupo'].'</li><br><li class="sub_me">';
				$retorno .= $this->MenuAssistentTela->_show($submenus[$i]['menugroup_id']);
