...
class AccessColumnsController extends AccessUsersAppController{
    ...
    /**
     * Método que configura a autorização de colunista
     */
    public function admin_enableDisabledColumnist($user_id)
    {
            if($this->GetPermissionColumns->start($user_id, 'col'))
            {
                 if($this->request->is('post') || $this->request->is('put'))
                 {
                     $data = $this->request->data;
                     if($this->SetAccessDataColumns->start($data, $user_id, 'col'))
                     {
                        $dataSave = $this->SetAccessDataColumns->dataSave; #pr($dataSave); exit(0);
                        $datasource = $this->AccessUser->getDataSource();
                         try{
                                ...
                                $this->redirect(array('action' => 'enableDisabledColumnist', $user_id));
                             }catch(Exception $e){
                                 $datasource->rollback();
     ...
     /**
     * Método que configura a autorização de administrador de coluna
     */
    public function admin_enableDisabledAdmColum($user_id)
    { 
            if($this->GetPermissionColumns->start($user_id, 'adm'))
            {
                 if($this->request->is('post') || $this->request->is('put'))
                 {
                     $data = $this->request->data;
                     if($this->SetAccessDataColumns->start($data, $user_id, 'adm'))
                     {
                        ...
                        $datasource = $this->AccessUser->getDataSource();
                         try{
                                 ...
                                 $datasource->commit();
                                 $this->Session->setFlash(__('Sucesso!!!'), 'default', array('class' => 'success'));
                                $this->redirect(array('action' => 'enableDisabledAdmColum', $user_id));
                             }catch(Exception $e){
                                 $datasource->rollback();
